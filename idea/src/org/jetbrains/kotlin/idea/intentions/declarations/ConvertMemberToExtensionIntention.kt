...
class ConvertMemberToExtensionIntention : SelfTargetingRangeIntention<KtCallableDeclaration>(KtCallableDeclaration::class.java, "Convert member to extension"), LowPriorityAction {
    ...
    override fun applyTo(element: KtCallableDeclaration, editor: Editor?) {
        val (extension, bodyToSelect) = createExtensionCallableAndPrepareBodyToSelect(element, true)
    ...
    private fun createExtensionCallableAndPrepareBodyToSelect(
        ...
        if (headerDeclaration != null) {
            val headerImplementations = headerDeclaration.headerImplementations().filterIsInstance<KtCallableDeclaration>()
            if (headerImplementations.isEmpty()) {
                val allowHeader = !isInteractive || askIfHeaderIsAllowed(element.containingKtFile)
                return processSingleDeclaration(element, allowHeader)
            }

            val declarationsToProcess = ArrayList<KtCallableDeclaration>().apply {
                add(headerDeclaration)
                addAll(headerImplementations)
                remove(element)
            }
            declarationsToProcess.forEach { processSingleDeclaration(it) }
        }

        return processSingleDeclaration(element)
    }
    ...
    companion object {
        fun convert(element: KtCallableDeclaration): KtCallableDeclaration {
            return ConvertMemberToExtensionIntention().createExtensionCallableAndPrepareBodyToSelect(element, false).first
        }
    }
}
